...
- `frontend/src/components/LiveMap.tsx`: Replaced DOM fire markers with a `himalayaair-fires` GeoJSON source and circle layer.
...
- `frontend/src/styles/global.css`: Removed obsolete DOM marker styles.
...
- Station and fire points now render inside the map canvas through native MapLibre/Mapbox layers.
...
- [x] Fire rendering moved from DOM markers to a native map layer.
...
- The map layer IDs are `himalayaair-stations-selected`, `himalayaair-stations-circles`, `himalayaair-stations-labels`, and `himalayaair-fires-circles`.
- Manual browser verification should focus on dragging, station click selection, mobile label density, heatmap toggle behavior, and fire layer toggle behavior at `http://localhost:3000`.
